...
#define S_FUNCTION_NAME forces_aero_msf
#endif
...
real_T rtP_A ; real_T rtP_CD ; real_T rtP_CL ; real_T rtP_rho ; const char *
...
mr_forces_aero_RegisterSimStateChecksum ( S ) ; ssSetNumSampleTimes ( S , 2 )
...
mr_forces_aero_MdlInfoRegFcn ( S , "forces_aero" , & retVal ) ; if ( ! retVal
) return ; }
#endif
#if SS_SFCN_FOR_SIM
...
( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( gix3e13jsxw ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { gix3e13jsxw * dw = ( gix3e13jsxw * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid ( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } b4osbtaao3 ( S , slmrGetTopTidFromMdlRefChildTid ( S , 0 , false ) , slmrGetTopTidFromMdlRefChildTid ( S , 1 , true ) , & ( dw -> rtm ) , & ( dw -> rtb ) , sysRanPtr , sysTid , ( ( NULL ) ) , ( ( NULL ) ) , 0 , - 1 ) ; ssSetModelMappingInfoPtr ( S , & ( dw -> rtm . DataMapInfo . mmi ) ) ; if ( S -> mdlInfo -> genericFcn != ( NULL ) ) { _GenericFcn fcn = S -> mdlInfo -> genericFcn ; } }
#define MDL_START
static void mdlStart ( SimStruct * S ) { mdlProcessParameters ( S ) ; }
static void mdlOutputs ( SimStruct * S , int_T tid ) { gix3e13jsxw * dw = ( gix3e13jsxw * ) ssGetDWork ( S , 0 ) ; real_T const * i_mmrcififrn = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ; real_T * o_B_1_1 = ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; real_T * o_B_1_2 = ( real_T * ) ssGetOutputPortSignal ( S , 1 ) ; if ( tid == PARAMETER_TUNING_TID ) { forces_aeroTID1 ( & ( dw -> rtb ) ) ; } if ( tid != CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { forces_aero ( i_mmrcififrn , o_B_1_1 , o_B_1_2 , & ( dw -> rtb ) ) ; } } static void mdlTerminate ( SimStruct * S ) { gix3e13jsxw * dw = ( gix3e13jsxw * ) ssGetDWork ( S , 0 ) ; mea4we2vim ( & ( dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_forces_aero_GetDWork ( ssGetDWork ( S , 0 ) ) ;
mxSetFieldByNumber ( ss , 0 , 1 , mdlrefDW ) ; } { mxArray * data =
mr_forces_aero_GetSimStateDisallowedBlocks ( ) ; mxSetFieldByNumber ( ss , 0
...
mr_forces_aero_SetDWork ( ssGetDWork ( S , 0 ) , mxGetFieldByNumber ( ss , 0
