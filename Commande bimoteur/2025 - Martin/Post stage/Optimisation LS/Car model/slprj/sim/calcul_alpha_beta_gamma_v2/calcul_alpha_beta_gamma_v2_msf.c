...
#define S_FUNCTION_NAME calcul_alpha_beta_gamma_v2_msf
#endif
...
real_T rtP_lf ; real_T rtP_lr ; real_T rtP_t ; const char *
...
static void mdlProcessParameters ( SimStruct * S ) { real_T * GlobalPrm_0 = ( real_T * ) NULL ; real_T * GlobalPrm_1 = ( real_T * ) NULL ; real_T * GlobalPrm_2 = ( real_T * ) NULL ; if ( ! ssGetModelRefGlobalParamData ( S , 0 , ( void * * ) ( & GlobalPrm_0 ) ) ) return ; if ( GlobalPrm_0 != NULL ) { ( void ) memcpy ( & ( rtP_lf ) , GlobalPrm_0 , sizeof ( real_T ) ) ; } if ( ! ssGetModelRefGlobalParamData ( S , 1 , ( void * * ) ( & GlobalPrm_1 ) ) ) return ; if ( GlobalPrm_1 != NULL ) { ( void ) memcpy ( & ( rtP_lr ) , GlobalPrm_1 , sizeof ( real_T ) ) ; } if ( ! ssGetModelRefGlobalParamData ( S , 2 , ( void * * ) ( & GlobalPrm_2 ) ) ) return ; if ( GlobalPrm_2 != NULL ) { ( void ) memcpy ( & ( rtP_t ) , GlobalPrm_2 , sizeof ( real_T ) ) ; } }
#endif
static void mdlPeriodicOutputUpdate ( SimStruct * S , int_T tid ) {
cbbpcstriyq * dw = ( cbbpcstriyq * ) ssGetDWork ( S , 0 ) ; real_T const *
i_dngh1j1lsh = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ; real_T const *
i_pfdaxu2ljt = ( real_T * ) ssGetInputPortSignal ( S , 1 ) ; real_T const *
i_p50ewvq5bi = ( real_T * ) ssGetInputPortSignal ( S , 2 ) ; real_T * o_B_4_1
= ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; real_T * o_B_4_2 = ( real_T
* ) ssGetOutputPortSignal ( S , 1 ) ; real_T * o_B_4_3 = ( real_T * )
ssGetOutputPortSignal ( S , 2 ) ; if ( tid == 0 ) {
calcul_alpha_beta_gamma_v2 ( i_dngh1j1lsh , i_pfdaxu2ljt , i_p50ewvq5bi ,
o_B_4_1 , o_B_4_2 , o_B_4_3 ) ; } } static void mdlInitializeSizes ( SimStruct
...
, 3 ) ) return ; if ( ! ssSetInputPortVectorDimension ( S , 0 , 4 ) ) return
...
mr_calcul_alpha_beta_gamma_v2_RegisterSimStateChecksum ( S ) ;
...
mr_calcul_alpha_beta_gamma_v2_MdlInfoRegFcn ( S ,
"calcul_alpha_beta_gamma_v2" , & retVal ) ; if ( ! retVal ) return ; }
#endif
#if SS_SFCN_FOR_SIM
...
( ! ssSetDataTypeSize ( S , mdlrefDWTypeId , sizeof ( cbbpcstriyq ) ) )
...
static void mdlSetupRuntimeResources ( SimStruct * S ) { cbbpcstriyq * dw = ( cbbpcstriyq * ) ssGetDWork ( S , 0 ) ; void * sysRanPtr = ( NULL ) ; int sysTid = 0 ; ssGetContextSysRanBCPtr ( S , & sysRanPtr ) ; ssGetContextSysTid ( S , & sysTid ) ; if ( sysTid == CONSTANT_TID ) { sysTid = 0 ; } lfcl4oswx4 ( S , slmrGetTopTidFromMdlRefChildTid ( S , 0 , false ) , slmrGetTopTidFromMdlRefChildTid ( S , 1 , true ) , & ( dw -> rtm ) , sysRanPtr , sysTid , ( ( NULL ) ) , ( ( NULL ) ) , 0 , - 1 ) ; ssSetModelMappingInfoPtr ( S , & ( dw -> rtm . DataMapInfo . mmi ) ) ; if ( S -> mdlInfo -> genericFcn != ( NULL ) ) { _GenericFcn fcn = S -> mdlInfo -> genericFcn ; } }
#define MDL_START
static void mdlStart ( SimStruct * S ) { mdlProcessParameters ( S ) ; }
static void mdlOutputs ( SimStruct * S , int_T tid ) { cbbpcstriyq * dw = ( cbbpcstriyq * ) ssGetDWork ( S , 0 ) ; real_T const * i_dngh1j1lsh = ( real_T * ) ssGetInputPortSignal ( S , 0 ) ; real_T const * i_pfdaxu2ljt = ( real_T * ) ssGetInputPortSignal ( S , 1 ) ; real_T const * i_p50ewvq5bi = ( real_T * ) ssGetInputPortSignal ( S , 2 ) ; real_T * o_B_4_1 = ( real_T * ) ssGetOutputPortSignal ( S , 0 ) ; real_T * o_B_4_2 = ( real_T * ) ssGetOutputPortSignal ( S , 1 ) ; real_T * o_B_4_3 = ( real_T * ) ssGetOutputPortSignal ( S , 2 ) ; if ( tid == PARAMETER_TUNING_TID ) { } if ( tid != CONSTANT_TID && tid != PARAMETER_TUNING_TID ) { calcul_alpha_beta_gamma_v2 ( i_dngh1j1lsh , i_pfdaxu2ljt , i_p50ewvq5bi , o_B_4_1 , o_B_4_2 , o_B_4_3 ) ; } } static void mdlTerminate ( SimStruct * S ) { cbbpcstriyq * dw = ( cbbpcstriyq * ) ssGetDWork ( S , 0 ) ; ph0uk5pae2 ( & ( dw -> rtm ) ) ; return ; }
...
mdlrefDW = mr_calcul_alpha_beta_gamma_v2_GetDWork ( ssGetDWork ( S , 0 ) ) ;
...
mr_calcul_alpha_beta_gamma_v2_SetDWork ( ssGetDWork ( S , 0 ) ,
