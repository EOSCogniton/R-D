...
[A, tUart] = picgui.padr(Rn,5,t_Rn);  % sample on last column data received (remove NaN numbers from the original matrix)
  
if ~isempty(A)
    clf 
    t = ([0:(length(A)-1)]  + tUart(1))*Tserial; % Do not rely on UART data arrival timestamp. Recreate time vector
    subplot(3,1,1); plot(t,A(:,2:3)); axis tight; ylabel('v_d_q (in V)'); ylim([-12 12]);    
    subplot(3,1,2); plot(t,A(:,4)); axis tight; ylabel('om (in rad/s)'); ylim([-380 380]);
    subplot(3,1,3); plot(t,A(:,5)); axis tight; ylabel('th (in rad)'); ylim([0 2*pi]);
    xlabel('time (in s)')
    dim=[0 .9 .1 .1] ;
    if(A(end,1)==0)
