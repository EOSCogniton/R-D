...
PWM_frequency 	= 12e3;             %Hz     // converter s/w freq
...
Ts_speed        = 1e-3;%10*Ts;        %Sec        // Sample time for speed controller
Ts_SerialCom =10*T_pwm;
...
target.model                = 'PIC32MK1024MCF';% Manufacturer Model Number
target.sn                   = '123456'; % Manufacturer Serial Number
target.CPU_frequency        = 120e6;    %(Hz)   // Clock frequency
...
%% SMO parameters
Vd_Ref_openLoop_PU = 0.15; % Use 0.12 for Dyno setup and 0.1 for others
%smo = mcb_SetSMOParameters;
smo.CurrentObsGain = 0.1;
smo.SlidingSurfaceLim = 0.6;
...
PI_params.Ki_id = 600;
PI_params.Ki_i = 600;
PI_params.Kp_speed = 0.2;
PI_params.Ki_speed  = 1.6;
